...
from phasespace.fromdecay import FullDecay
...
def check_norm(full_decay: FullDecay, **kwargs) -> list[tuple]:
    """Helper function that checks whether the normalize_weights argument works for FullDecay.generate.
    Args:
...
def test_single_chain():
    ...
    container = FullDecay.from_dict(example_decay_chains.dplus_single, tolerance=1e-10)
...
def test_branching_children():
    ...
    container = FullDecay.from_dict(example_decay_chains.pi0_4branches, tolerance=1e-10)
...
def test_branching_grandchilden():
    ...
    container = FullDecay.from_dict(example_decay_chains.dplus_4grandbranches)
    output_decays = container.gen_particles
    assert len(output_decays) == 4
    assert_almost_equal(sum(d[0] for d in output_decays), 1)
    check_norm(container, n_events=1)
...
def test_big_decay():
    container = FullDecay.from_dict(example_decay_chains.dstarplus_big_decay)
    output_decays = container.gen_particles
    assert_almost_equal(sum(d[0] for d in output_decays), 1)
    check_norm(container, n_events=1)
    (normed_weights, events), _ = check_norm(container, n_events=100)
